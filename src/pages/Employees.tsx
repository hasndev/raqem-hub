...
const Employees = () => {
  ...
  const [formData, setFormData] = useState({
    name: "", email: "", phone: "", department_id: "", position: "", salary: "", start_date: "", status: "active" as Employee["status"]
  });

  const filteredEmployees = employees.filter(e => e.name.includes(search) || e.position?.includes(search));
  
  const resetForm = () => setFormData({
    name: "", email: "", phone: "", department_id: "", position: "", salary: "", start_date: "", status: "active"
  });
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <select
          ...
          className="w-full h-10 px-4 bg-muted border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/20"
        >
          {Object.entries(employeeStatuses).map(([key, val]) => (
  ...
  return (
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="relative max-w-md animate-slide-up">
          ...
          <input
            ...
            className="w-full h-10 pr-10 pl-4 bg-card border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/20"
          />
        </div>
            ...
            <tbody className="divide-y divide-border">
              {filteredEmployees.map((emp, index) => (
                <tr key={emp.id} className="hover:bg-muted/30 transition-colors animate-fade-in" style={{ animationDelay: `${index * 50}ms` }}>
                  ...
                  <td className="px-6 py-4">
                    <Badge className={employeeStatuses[emp.status].color}>{employeeStatuses[emp.status].label}</Badge>
                  </td>
                  <td className="px-6 py-4">
      ...
      <Modal isOpen={isAddOpen} onClose={() => { setIsAddOpen(false); resetForm(); }} title="إضافة موظف جديد">
        {renderForm(handleAdd, "إضافة الموظف")}
      </Modal>
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <Badge className={employeeStatuses[selectedEmployee.status].color + " mt-2"}>{employeeStatuses[selectedEmployee.status].label}</Badge>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
              ...
              <div>
                ...
                <p className="font-bold text-xl text-primary">${selectedEmployee.salary.toLocaleString()}</p>
              </div>
            </div>
          </div>
        )}
